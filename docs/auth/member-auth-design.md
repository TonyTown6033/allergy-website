...
结合本轮代码 review，后续需要把以下约束补成硬规则，而不是停留在 controller 里的软校验：

- 会员邮箱唯一性需要由数据库唯一索引保证，不能只依赖注册前查询
- 用户名命名空间需要避开已有邮箱值，避免 `username/email` 混合登录时发生歧义匹配
- 找回密码接口对外应返回统一错误语义，避免暴露管理员账号、非会员账号或禁用状态
- 公共站所有登录入口都需要保留来源页的查询参数，避免跳转后丢失关键上下文

这些项已经同步登记到 [TODO.md](../../TODO.md)，后续修正时应优先补齐对应测试。
