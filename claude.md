...
如果任务涉及会员注册 / 登录 / 找回密码，再补读：

- `./docs/auth/member-auth-design.md`
- `./docs/auth/member-auth-api-contract.md`
- `./docs/auth/member-auth-migration-cleanup.md`
...
- 已有履约闭环的回归：
  - 订单创建
  - 支付回调后的订单状态更新
  - 发货 / 样本回寄 / 样本签收 / 开始检测 / 报告发布 / 补发邮件 / 完成订单
  - 时间线事件写入
- 用户名 / 邮箱 + 密码登录流程
- 注册时邮箱验证流程
- 找回密码与重置密码流程
...
这个工作区后续新增功能，默认按“先落文档，再进实现”的方式推进。
...
各领域目录里的 `_feature-template.md` 可直接复制作为新功能起稿。
...
以下能力已经落地，后续任务默认是在这个基础上继续扩展，而不是推倒重做：
...
- 管理后台已存在以下履约动作：
  ...
  - 补发报告邮件
  - 完成订单
- 用户端已有订单时间线、支付状态、报告入口

### 4.2 当前优先级

根据 `TODO.md`，当前优先处理这些缺口：
...
以下内容默认已经具备或方案已定，不要重复造一套：

- 管理后台过敏原订单页面骨架
- 已有履约状态机和时间线链路
- PDF 报告版本化、发布、补发邮件的基础能力
- 以 `Option JSON` 驱动的第一阶段站点内容
...
- 上述账号方案是目标态，不代表当前代码已完成
- 当前代码仍以邮箱验证码登录为主
- 后续涉及登录改造时，应按这里的目标态推进，不要再以“首次登录自动建号”继续扩展
...
- 订单履约后台页面和核心动作已经存在
- 后续优先做增强项，例如退款、审计、备注流转、列表快捷动作，而不是重写现有履约模块
...
- 管理后台当前也支持管理员态预览 / 下载报告
- 后续若调整报告模块，应保持“版本历史 + 当前有效版本”这条语义不变
...
- 现在不要优先开发 AI 功能
- 第一阶段闭环已基本跑通
- 现在优先补财务、售后、账号体系，不要把精力转去做 AI 功能
